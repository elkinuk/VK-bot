...
export class App {
	...
	static add_user(uid,phone,first_name,last_name) {
		if(uid){
			...
			App.check_user(uid,()=>{
				...
				Sub.require_to_bd('insert?', $.param({
					table: 'Users',
					field: 'id,first_name,last_name,phone',
					val: [uid,first_name,last_name,phone].join('","')
				}));
	...
	static add_order(uid,phone,details){
		if(uid){
			App.check_user(uid, ()=>{}, ()=>{ Bot.get_name(uid,phone,App.add_user) });
			Sub.require_to_bd('insert?', $.param({
				table: 'Orders',
